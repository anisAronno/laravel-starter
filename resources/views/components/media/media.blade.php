...
@if ($visible == true)
    <div
        class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 grow gap-5 p-5 bg-gray-100 dark:bg-gray-900">
        @foreach ($medias as $media)
            <div
                ...
                <img src="{{ Media::getURL($media->url) }}" alt="{{ $media->title }}" class="w-full h-auto image-item object-cover">
            </div>
        @endforeach
    </div>
